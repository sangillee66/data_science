...
데이터사이언스 과정의 각 단계는 특징적인 과업으로 구성되어 있고, 거기에 맞춰 특징적인 함수들을 장착한 패키지들이 개발되어 왔다. 그런데 최근 RStudio는 데이터사이언스와 밀접히 관련된 패키지들을 모아 일종의 엄브렐러 패키지인 [`tidyverse`](https://www.tidyverse.org/)를 만들어 제공하고 있다. tidyverse 패키지는 패키지의 패키지인데, 핵심적인 구성 패키지에 다음과 같은 것들이 있다.
...
특정 열(변수)에 의거해 조건을 만족하는 행을 솎아낸다. 함수가 무슨 일을 한 것인지 살펴보라.
...
특정 열(변수)에 의거해 행의 순서를 바꾼다. 함수가 무슨 일을 한 것인지 살펴보라.
...
특정 열(변수)에 의거해 (중복을 제거한) 고유한 행을 솎아 낸다. 함수가 무슨 일을 한 것인지 살펴보라.
...
열 중 일부를 솎아낸다. 무엇이 함수가 무슨 일을 한 것인지 살펴보라.
...
기존의 열을 변형하여 새로운 열을 생성한다. 함수가 무슨 일을 한 것인지 살펴보라.
...
열의 이름을 변경한다. 함수가 무슨 일을 한 것인지 살펴보라.
...
열의 위치(순서)를 변경한다. 함수가 무슨 일을 한 것인지 살펴보라.
...
열에 의거해 전체 행을 그룹으로 분할한다. 함수가 무슨 일을 한 것인지 살펴보라.
...
열의 통계량을 산출하여 새로운 열에 저장한다. 함수가 무슨 일을 한 것인지 살펴보라.

```{r}
summarize(flights, delay = mean(dep_delay, na.rm = TRUE))
```

위의 `group_by()` 함수와 결합하면, 그룹별로 열의 통계량을 산출하여 새로운 열에 저장할 수 있다. 함수가 무슨 일을 한 것인지 살펴보라.
