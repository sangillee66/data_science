...
![**Quarto**의 기본 개념(<https://rstudio.github.io/cheatsheets/html/quarto.html>)](https://rstudio.github.io/cheatsheets/html/images/quarto-illustration.png){#fig-quarto-1}
...
워드프로세서처럼 텍스트를 작성한다. 섹션 헤더, 하이퍼링크(hyperlink), 이미지 등을 지정할 수 있고, 인라인 코드 청크(inline code chunk)도 삽입할 수 있다. 마크다운 텍스트의 작성에 비주얼 에디터는 큰 역할을 한다. 비주얼 에디터는 마크다운 언어의 사용자 편의의성을 한번 더 강화한 것으로 볼 수 있다.
...
![](https://quarto.org/docs/visual-editor/images/visual-editing-format-menu.png){fig-align="center" width="200"}

![](https://quarto.org/docs/visual-editor/images/visual-editing-insert-menu.png){fig-align="center" width="200"}

![](https://quarto.org/docs/visual-editor/images/visual-editing-table-menu.png){fig-align="center" width="200"}
...
![](images/clipboard-2345291333.png){fig-align="center"}
...
다음의 [웹사이트](https://quarto.org/docs/publishing/quarto-pub.html)를 참고한다.

참고로 작년에 이 수업을 수강한 조교가 만든 블로그는 아래와 같다..

![예시 블로그(<https://geodyadeun.quarto.pub/blog_test>)](images/clipboard-2196527765.png){fig-align="center"}
...
예시 대시보드(<https://geodyadeun.quarto.pub/dashboard_test/>)에 접속하여 구성을 살펴본다. 다음의 사항에 주목한다.

-   내비게이션바에 서울대학교 로고가 있고, 오른쪽 맨 끝애 blog 및 이메일 아이콘이 위치해 있다.
...
format: 
  dashboard:
    logo: snu_ui_download.png
    nav-buttons: 
      - icon: house
        href: https://blog.naver.com/961102
        aria-label: Blog
      - icon: envelope
        href: mailto:geodyadeun@snu.ac.kr
        aria-label: Mail
    # scrolling: true
theme: default
editor: visual
...
::: callout-warning
아래 그래프, 테이블, 지도, 밸류박스 등 제시된 코드를 실행하기 위해서는, 코드 청크 내에서 코드가 실행되어야 함을 잊지 말자! (코드 맨 앞에 \`\`\`{r}를 입력하고, 맨 뒤에도 닫아준다.)
:::
...
-   프롬프트에 다음과 같이 입력하고 실행한다: quarto publish quarto-pub (qmd 파일만 있는 상태로 퍼블리쉬하기 위해서는, 'quarto publish quarto-pub 파일이름.qmd'를 입력한다.)

-   나머지 사항에 대해서는 조교의 도움을 받는다.

-   참고로 작년에 이 수업을 수강한 조교가 기말 발표를 위해 만든 대쉬보드는 아래와 같다.

![예시 대시보드(<https://geodyadeun.quarto.pub/geoclass_dyadeuni>)](images/clipboard-2706418153.png)
