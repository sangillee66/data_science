...
여기서는 R로 데이터사이언스를 하는 과정 중 데이터 불러오기(importing)와 데이터 정돈하기(tidying)에 집중한다. 아래의 @fig-4-1 에서 보는 바처럼, 불러오기와 데이터 정돈하기는 데이터사이언스 프로세스의 첫 두 단계에 해당한다.

![데이터사이언스 프로세스와 불러오기 및 정돈하기(<https://r4ds.hadley.nz/import.html>)](images/data_science_process_import-01.png){#fig-4-1}
