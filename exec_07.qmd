...
두 번째 방식은 `kosis` 패키지의 `getStatDataFromURL()` 함수를 활용하는 것이다. KOSIS의 데이터 서비스 구조를 감안할 때 이 것이 좀 더 용이한 방식일 수 있다. 데이터 추출은 다음과 같이 두 단계의 과정을 거친다.
...
::: callout-tip
이렇게 수집된 문자열 데이터는 텍스트 마이닝(text mining)을 위한 중요한 원천이 되는데, 분석을 위해서는 데이터 정처리 과정이 필수적이다. 다음의 링크를 참고할 수 있다. <https://r2bit.com/book_tm/> 또한 다음의 책을 참고할 수 있다([@gimyeongu2021])
:::
