...
-   [`left_join()`](https://dplyr.tidyverse.org/reference/mutate-joins.html): 첫 번째 변수는 그대로 둔 상태에서 두 번째 변수를 결합함으로써 두 번째 변수의 열을 가져옴

-   [`right_join()`](https://dplyr.tidyverse.org/reference/mutate-joins.html): 두 번째 변수는 그대로 둔 상태에서 첫 번째 변수를 결합함으로써 첫 번째 변수의 열을 가져옴

-   [`inner_join()`](https://dplyr.tidyverse.org/reference/mutate-joins.html): 두 변수 모두에 존재하는 열을 취함

-   [`full_join()`](https://dplyr.tidyverse.org/reference/mutate-joins.html): 최소한 한 변수에 존재하는 열을 모두 취함

-   [`semi_join()`](https://dplyr.tidyverse.org/reference/filter-joins.html): 첫 번째 변수의 행 중 두 번째 변수에 대응하는 행이 있는 것만 취함

-   [`anti_join()`](https://dplyr.tidyverse.org/reference/filter-joins.html): 첫 번째 변수의 행 중 두 번째 변수에 대응하는 행이 없는 것만 취함
