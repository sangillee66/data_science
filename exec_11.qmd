...
반응형 표현식 함수에 포함되는 것에 다음과 같은 함수가 포함된다.

-   `reactive({})`: 대표적인 일반 반응형 표현식 함수

-   `reactiveVal()`, `reactiveValues()`: 사용자가 직접 저장하고 갱신하는 값의 '저장소' 함수
...
`reactive({})` 계열 함수에 포함되는 함수들에 `reactiveVal()`, `reactiveValues()`가 있다. `reactive()` 함수가 입력이 바뀌면 자동으로 갱신되는 '계산식'인데 반해, `reactiveVal()`과 `reactiveValues()`는 사용자가 직접 저장하고 갱신하는 값의 '저장소'이다.

`reactiveVal()`은 하나의 스칼라 값을 저장하고 갱신하는 반응형 변수이다. 아래에 예시가 있다.
...
`eventReactive(``eventExpr, handlerExpr, ...``)`는 이를 방지하기 위해 특정 이벤트가 발생할 때만 반응형 표현식을 갱신하고 그 외의 입력 변화는 무시하는 구조를 제공한다. 인수인 `eventExpr`에 트리거 역할을 하는 이벤트 표현식이 지정되는데, 주로 `input$button`, `input$go`, `input$file`처럼 값 자체보다 변경 이벤트 발생이 중요한 입력 요소로 사용된다. `handlerExpr`은 이벤트가 발생했을 때 실행되는 핸들러 표현식(계산식)이다. 아래에 예시가 있다.
...
반응형 관찰자 함수는 반응형 값(`*Input()`, `reactive()`, `reactiveVal()`, `reactiveValues()` 등)의 변화가 발생했을 때 특정 동작(side effect)을 수행하는 반응형 구성 요소이다. 특정한 트리거에 반응(triggering reactions)하는 함수이다. 반응형 관찰자 함수는 값을 반환하지 않고, 대신 다음과 같은 동작을 행한다.
...
대표적인 함수에 `observe()` 와 `observeEvent()` 가 있다. `observe()` 는 블록 내부에서 읽은 모든 반응성 입력값에 자동으로 의존하며, 의존성이 자동 추적되므로 가장 기본적인 함수이다. 아래의 예시에서 보면 사용자가 `input$region`을 바꿀 때마다 자동 실행된다.
...
`observeEvent()` 함수는 "이벤트(트리거)"가 발생할 때문 실행되며, 함수에 지정한 값(보통 입력 위젯)만을 트리거로 사용한다. 내부에서 참조하는 다른 값들은 트리거로 작동하지 않는다. `eventReactive()` 함수와 마찬가지로 `observeEvent(eventExpr, handlerExpr, ...)` 형태로 사용된다. `eventExpr` 인수는 이벤트 표현식을 지정하는데, 보통 `input$button`, `input$go`, `input$file`과 입력 함수가 설정된다. `handlerExpr`는 핸들러 표현식으로 이벤트가 발생했을 때 실행하는 부수효과 즉 '활동'이 지정된다.
...
반응형 유틸리티 함수(reactive utilities)는 반응형 표현식 함수나 반응형 관찰자 함수 내부에서 반응성의 작동 방식을 제어하거나, 예외 처리(validation), 성승/흐름 제어(throttle/debounce), 종속성 무시(isolate), 요구 조건 검사(req) 등을 수행하는 보조 함수를 말한다. 이 함수들은 새로운 반응형 값을 생성하지 않고, 반응성을 보조하거나, 억제하거나, 예외를 조절하는 함수이다.
