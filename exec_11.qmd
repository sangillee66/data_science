...
@fig-reactive_flow 를 보면, Shiny의 반응성 플로에서 다양한 반응성 함수가 사용되고 있음을 알 수 있다. 이러한 Shiny의 반응성 함수는 **역할에 따라 크게 다섯 부류**로 나눌 수 있다.

#### 입력 함수: `*Input()` 계열

입력 함수는 입력 위젯을 통해 사용자의 입력값을 받는데, 그 값은 `input` 객체에 저장된다. `input` 객체는 리스트처럼 보이는 반응형 값(reactive value)이며, 웹 브라우저에서 전달된 입력에 의해 설정되는 여러 개의 개별 반응형 값을 포함하고 있다. 서버에서 `input$*`과 같은 형태로 호출된다.
...
반드시 `output$* <- render*({ ... })` 형태로 사용되며, 출력의 최종 종착점이며**,** UI의 `*Output()` 함수와 일대일로 대응된다. 이래의 예시에서, `renderPlot({})` 함수는 플롯 형식을 출력을 생성하고 그것을 `output` 객체에 `distPlot`이라는 ID로 저장한다. 이것은 UI의 출력 함수를 통해 호출되어 메인 콘텐츠 영역에 표출된다.
...
`reactive({})` 함수는 입력값이 바뀔 때마다 자동으로 계산되기 때문에, 예를 들어 슬라이더를 움직일 때마다 모델을 다시 돌린다거나, 입력창을 타이핑할 때마다 무거운 연산을 반복한다거나 하는 비효율적인 상황이 발생할 수 있다. `eventReactive(source, {handler})`는 이를 방지하기 위해 특정 이벤트(`source`)가 발생할 때만 반응형 표현식을 갱신하고 그 외의 입력 변화는 무시하는 구조를 제공한다. 아래에 예시가 있다.
...
입력 함수 `actionButton(inputID = "go", ...)`로부터 버튼을 누를 때 마다, 즉 `input$go` 이벤트가 발생할 때에만 `result()`를 재계산한다. `input$x`가 변한다고 자동적으로 갱신하지는 않는다.
...
`observeEvent(source, {handler})` 함수는 "이벤트(트리거)"가 발생할 때문 실행되며, source에 지정한 값(보통 입력 위젯)만을 트리거로 사용한다. 내부에서 참조하는 다른 값들은 트리거로 작동하지 않는다. 특정 이벤트 발생 시에만 실행되고, 불필요한 재실행을 방지하기 위해 종속성 자동 추적을 하지 않는다. 버튼 클릭, 발짜 선택, 파일 업로드 등 "이벤트 기반" 작업에 매우 적합하다.
