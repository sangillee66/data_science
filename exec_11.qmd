...
UI 부분은 다음과 같은 구조를 가지고 있다.

첫째, 전체적으로 `fluidPage()`라는 함수에 의해 규정되고, `fluidPage()`는 다시 `titlePanel()`과 `sidebarLayout()`으로 나뉘고, `sidebarLayout()`은 다시 `sidebarPanel()`과 `mainPanel()`로 나뉜다.

-   `fluidPage()` : UI의 최상위 컨테이너로서 전체 레이아웃을 관장하며, 브라우저 화면 크기에 따라 자동으로 늘어나고 줄어드는 반응형 레이아웃을 제공한다. 이러한 특성 때문에 ‘fluid’라는 이름이 붙었으며, 고정 폭을 가진 레이아웃은 `fixedPage()`를 통해 정의할 수 있다. Shiny 앱에서는 기본적으로 `fluidPage()`를 사용한다고 생각하면 된다.

    -   `titlePanel()` : 제목이 들어가는 부분으로, 생략할 수 있다.
...
> 결국 UI는 입력을 받고 출력을 표출하는데, 전자는 `sliderInput()`와 같은 입력 함수를 통해 이루어지며, 후자는 `plotOutput()`과 같은 출력 표출 함수를 통해 이루어진다. 이들 함수는 출력과 입력의 형식에 따라 일반적으로 `*Input()`과 `*Output()`의 형태를 띤다.
...
둘째, 출력물은 `renderPlot({})`과 같은 **출력 생성 함수**를 통해 이루어진다. 이렇게 생성된 출력은 '이름'이 지정되어 Server의 `output` 객체의 한 원소로 저장된다. 예를 들어, 코드에서 `output$distPlot`은 생성된 출력에 `"distPlot"`이라는 이름을 부여하고 이를 `output` 객체에 저장함을 의미한다. 이 이름은 UI 부분의 `plotOutput(outputId = "distPlot")`에서 사용되며, 이를 통해 해당 출력이 UI에 표출된다.
...
위의 두 가지 방식을 결합한 제3의 길도 있다. 우선 위의 분리 파일 방식과 같이 UI와 Server 부분에 대한 파일을 개별적을 생성한다. 차이점은 반드시 ui와 server 객체를 할당하는 방식으로 일어져야 한다는 점이다. my_ui.R의 내용은 다음과 같다. 이 방식에서는 UI 부분의 파일 이름이 반드시 ui.R일 필요가 없다.
...
그리고 나서 단일 파일 방식과 같이 app.R 파일을 통해 결합한다. 내용은 다음과 같다.
...
UI 파일과 Server 파일을 분리하는 방식은 코드가 매우 복잡할 때 사용하는 것으로 보통을 단일 파일 방식을 사용한다.

## 입출력 함수의 종류

### 입력 함수

입력 함수 혹은 입력 위젯 생성 함수의 종류는 다양한데, [R Shiny Components](https://shiny.posit.co/r/components/) 웹페이지 기준으로 대략 25개 정도이다. @fig-widget 은 그 중 일부를 보여주고 있다.
...
둘째, 입력 함수에 따라 인수의 종류가 다르다. 자세한 사항은 [R Shiny Components](https://shiny.posit.co/r/components/) 웹페이지를 참조한다.
...
Shiny 웹 앱에서의 활용을 염두에 둔 패키지들은 모두 이러한 출력 함수의 쌍을 제공하고 있다. 따라서 출력 함수는 굉장히 많을 수 있다. [R Shiny Components](https://shiny.posit.co/r/components/) 웹페이지 기본 5개의 함수쌍에 주요 패키지를 포함하는 12개 정도의 출력 함수쌍을 예시로 제공하고 있다.
