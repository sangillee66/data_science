...
## 최신 레이아웃 디자인: bslib 패키지

### 개요

위에서 Shiny의 전체 레이아웃은 위계적으로 구성된 레이아웃 함수들에 의해 결정된다고 했다. `fluidPage()`, `titlePanel()`, `sidebarLayout()`, `sidebarPanel()`, `mainPanel()` 과 같은 레이아웃를 통해 @fig-first 에 나타나 있는 것과 같은 외향의 레이아웃이 완성된 것이다.

이러한 **전통적인** 레이아웃 디자인을 보다 현대적인 레이아웃으로 탈바꿈하기 위해 등장한 것이 [**bslib**](https://rstudio.github.io/bslib/) 패키지이다. [**bslib**](https://rstudio.github.io/bslib/) 패키지는 Bootstrap을 기반으로 Shiny와 R Markdown을 위한 현대적인 UI 도구 모음을 제공한다. 카드, 밸류 박스, 사이드바 등 재사용 가능한 UI 구성 요소를 통해 Shiny 웹 앱과 문서를 효율적으로 구성할 수 있으며, 테마 시스템을 활용해 웹 앱과 문서의 외관을 유연하게 사용자 정의하고 실시간으로 조정할 수 있다. 또한 최신 Bootstrap 및 Bootswatch를 지원하여, 기본적으로 Bootstrap 3에 의존하는 기존 Shiny 및 R Markdown 환경보다 현대적인 웹 UI 구현을 가능하게 한다.

[**bslib**](https://rstudio.github.io/bslib/) 패키지는 shiny 패키지의 기본 레이아웃 함수와는 다른 종류의 함수를 제공한다. 그러므로 조금 더 현대적인 감각의 웹 앱을 생성하기 위해서는 [**bslib**](https://rstudio.github.io/bslib/) 패키지에서 제공되는 레이아웃 함수에 익숙해져야 한다. 여기서는 [**bslib**](https://rstudio.github.io/bslib/) 패키지를 통해 @fig-first 웹 앱이 어떻게 달라지는지는 살펴보고자 한다.

우선 [**Shiny**](https://rstudio.github.io/shiny/) 패키지는 기본적으로 11개의 예제 웹 앱을 제공한다. 다음과 같이 실행해 본다.
...
이 들 중 첫 번째 예제인 "01_hello"가 @fig-first 웹 앱이다. 해당 예제를 실행하려면 다음과 같이 하면 된다.
...
아래의 @fig-bslib-1 과같은 웹 앱이 생성된다. 이것을 @fig-first 와 비교해 보라. 조금 달라 보이며, 왼편 슬라이더바에서 \< 를 누르면 입력 사이드바를 숨길 수 있다.

![[**bslib**](https://rstudio.github.io/bslib/) 패키지를 활용한 웹 앱의 예제](images/clipboard-1762722241.png){#fig-bslib-1}

코드도 동시에 살펴보려면 다음과 같이 실행하면 된다.
...
삽입되어 있는 설명 부분을 제거하고 코드만 남기면 다음과 같다. 이것을 @fig-first 의 코드와 비교해 보라. 오로지 UI 부분의 코드만 다르고, 그것도 레이아웃 함수명만 달라져 있다는 것을 알 수 있다(입출력 함수는 그대로이다).
...
첫째, `page_sidebar()`가 최상위 레이아웃 함수이다. `page_sidebar()`는 [**bslib**](https://rstudio.github.io/bslib/)에서 가장 널리 사용되는 페이지 레이아웃 함수로, 전체 너비를 차지하는 헤더(제목)와 사용자 입력을 위한 사이드바를 갖춘 대시보드형 레이아웃을 손쉽게 구성할 수 있도록 설계되었다. `page_sidebar()`함수는 전통적인 shiny 레이아웃 함수 중 `fluidPage()`, `titlePanel()`, `sidebarLayout()` 함수의 역할을 동시에 수행한다고 생각할 수 있다.

::: callout-note
## page_fluid() 함수

사실은 `page_fluid()` 함수가 `fluidPage()` 를 대신하고, `page_sidebar()` 함수가 `page_fluid()` 함수의 차하 위계 함수로 기능하면서 `titlePanel()`과 `sidebarLayout()` 함수를 대신하지만, 거의 무조건 `page_sidebar()` 함수로 시작한다.
:::
...
-   `sidebar = sidebar()`: `sidebarPanel()` 함수와 동일한 기능을 한다. `sidebarPanel()` 함수 속에서 입력 함수인 `siderInput()` 함수가 들어가 있었던과 동일하게 `sidebar()` 속에 `siderInput()` 함수가 들어가 있다.

셋째, 전통적인 Shiny의 `mainPanel()` 함수 없이 출력 표출 함수(`plotOutput()`)가 막바로 사용된다. 그러나 이것은 가장 단순한 방식으로, `card()` 함수를 사용하면 보다 풍성한 옵션을 사용할 수 있다. `card()` 함수 내부에서 `card_header()`와 `card_body()`라는 컴포넌트 함수를 활용한다.
...
다음과 같이 용도별 레이아웃 함수를 정리할 수 있다. 자세한 사항은 [**bslib**](https://rstudio.github.io/bslib/) 패키지 홈페이지의 [Get Started](https://rstudio.github.io/bslib/articles/dashboards/index.html)나 [Layouts](https://rstudio.github.io/bslib/articles/column-layout/index.html)를 참고할 수 있다.

+----------------------------+---------------------------+-------------------------------------+
| 용도                       | 함수                      | 위치 및 특징                        |
+============================+===========================+=====================================+
| 단일 페이지(사이드바 포함) | `page_sidebar()`          | 최상단                              |
|                            |                           |                                     |
|                            | `layout_siderbar()`       | `card()` 내부                       |
+----------------------------+---------------------------+-------------------------------------+
| 다중 컬럼                  | `layout_columns()`        | `page_sidebar()` 내부               |
|                            |                           |                                     |
|                            | `layout_column_wrap()`    |                                     |
+----------------------------+---------------------------+-------------------------------------+
| 다중 페이지                | `page_navbar()`           | 최상단                              |
|                            |                           |                                     |
|                            |                           | `nav_panel()`이 개별 페이지         |
+----------------------------+---------------------------+-------------------------------------+
| 다중 패널(페이지 수준)     | `navset_tab()`            | `page_sidebar()` 내부               |
|                            |                           |                                     |
|                            | `navset_pill()`           | `nav_panel()`이 개별 패널           |
|                            |                           |                                     |
|                            | `navset_underline()`      |                                     |
+----------------------------+---------------------------+-------------------------------------+
| 다중 패널(card 수준)       | `navset_card_tab()`       | `page_sidebar()` 이나 `card()` 내부 |
|                            |                           |                                     |
|                            | `navset_card_pill()`      | `nav_panel()`이 개별 패널           |
|                            |                           |                                     |
...
가장 중요한 컴포넌트에 카드(card), 사이드바(sidebar), 밸류 박스(value box), 툴팁(tooltip)과 팝오버(popover)가 있다. 자세한 사항은 [**bslib**](https://rstudio.github.io/bslib/) 패키지 홈페이지의 [Components](https://rstudio.github.io/bslib/articles/cards/index.html)를 참고할 수 있다.

**카드**(card)는 현대적인 사용자 인터페이스(UI)에서 가장 널리 사용되는 정보 구성 단위 중 하나이다. 기본적으로 카드는 테두리와 여백을 갖는 직사각형 컨테이너에 불과하지만, 관련된 정보를 의미 있게 묶어 제시할 경우 사용자가 내용을 보다 쉽게 이해하고, 집중하며, 탐색할 수 있도록 돕는다. 이러한 특성 때문에 카드는 대시보드와 UI 설계에서 핵심적인 역할을 하며, 대부분의 성공적인 대시보드 및 UI 프레임워크는 카드를 주요 컴포넌트로 채택하고 있다. card() 함수는 다음과 같은 컴포넌트 함수를 갖는다.
...
**밸류 박스**(value box)는 대시보드에서 핵심적인 수치나 지표를 한눈에 전달하기 위해 사용되는 UI 컴포넌트이다. 값(value), 설명(label), 아이콘(icon) 등을 함께 제시하여 사용자가 현재 상태나 주요 변화를 빠르게 파악할 수 있도록 돕는다. [**bslib**](https://rstudio.github.io/bslib/)의 `value_box()`는 카드(card) 기반 설계를 따르므로 다른 UI 요소들과 시각적으로 일관되게 어우러지며, Shiny의 반응성과 결합해 값이 동적으로 갱신되는 대시보드를 구성하는 데 특히 유용하다.

`value_box()`는 네 가지 주요 구성 요소로 이루어진다.
