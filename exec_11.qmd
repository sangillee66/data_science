...
app.R에서 스크립트 윈도우 오른쪽 상단에 위치한 Run App 버튼을 클릭하면 @fig-first 과 같은 웹 앱이 새로운 윈도우에 생성된다. 왼편의 슬라이더바를 움직이면 오른편의 히스토그램이 변한다는 점을 확인한다. 즉 빈(bin)의 개수를 달리하면서 데이터 분포를 탐색해보기 위한 단순한 웹 앱이 만들어진 것이다.
...
둘째, 출력물은 `renderPlot({})`과 같은 **출력 생성 함수(렌더링 함수)**를 통해 이루어진다. 이렇게 생성된 출력은 '이름'이 지정되어 Server의 `output` 객체의 한 요소로 저장된다. 예를 들어, 코드에서 `output$distPlot`은 생성된 출력에 `"distPlot"`이라는 이름을 부여하고 이를 `output` 객체에 해당 이름으로 저장함을 의미한다. 이 이름은 UI의 출력 표출 함수 `plotOutput(outputId = "distPlot")`에서 사용되며, 이를 통해 해당 출력이 UI에 표출된다.
...
> 결국 Server는 UI에서 전달된 입력에 기반하여 출력을 산출한다. 구체적으로, `input$*` 형식의 입력값을 Server 내부에서 사용하여 `renderPlot({})`과 같은 출력 생성 함수를 통해 출력을 생성하고, 그 결과를 `output$*` 형식으로 저장함으로써 UI에 표출될 수 있도록 한다.
...
+--------------------+--------------------------------------+------------------------+
| 형식               | 출력 표출 함수(UI)                   | 출력 생성 함수(Server) |
+====================+======================================+========================+
...
: 기본 출력 함수 {#tbl-out1 tbl-colwidths="\[25, 50, 25\]"}
...
+----------------------------------------------------+----------------------------------+------------------------+
| 패키지                                             | 출력 표출 함수(UI)               | 출력 생성 함수(Server) |
+====================================================+==================================+========================+
...
: 패키지 출력 함수 {#tbl-out2 tbl-colwidths="\[25, 50, 25\]"}
...
[**bslib**](https://rstudio.github.io/bslib/) 패키지는 shiny 패키지의 기본 레이아웃 함수와는 다른 종류의 함수를 제공한다. 그러므로 조금 더 현대적인 감각의 웹 앱을 생성하기 위해서는 [**bslib**](https://rstudio.github.io/bslib/) 패키지에서 제공되는 레이아웃 함수에 익숙해져야 한다. 여기서는 [**bslib**](https://rstudio.github.io/bslib/) 패키지를 통해 @fig-first 웹 앱이 어떻게 달라지는지를 살펴본다.
...
삽입되어 있는 설명 부분을 제거하고 코드만 남기면 다음과 같다. 이것을 @fig-first 의 코드와 비교해 보라. 오로지 UI 부분의 코드만 다르고, 그것도 레이아웃 함수명만 달라져 있다는 것을 알 수 있다(입출력 함수는 그대로이다). 결국 [**bslib**](https://rstudio.github.io/bslib/) 패키지는 UI 부분의 레이아웃만 바꾼다.
...
| 용도              | 함수                      | 위치 및 특징                              |
+===================+===========================+===========================================+
| 단일 페이지       | `page_fluid()`            | 기본형                                    |
|                   |                           |                                           |
|                   | `page_fillable()`         | 대시보드형                                |
|                   |                           |                                           |
|                   | `page_sidebar()`          | 표준형(전역적 사이드바 포함)              |
+-------------------+---------------------------+-------------------------------------------+
| 사이드바 레이아웃 | `layout_siderbar()`       | `page_*()` 내부                           |
|                   |                           |                                           |
|                   |                           | `card()` 내부                             |
|                   |                           |                                           |
|                   |                           | `nav_panel()` 내부                        |
...
+-------------------+---------------------------+-------------------------------------------+
| 다중 페이지       | `page_navbar()`           | 네비게이션바 레벨                         |
|                   |                           |                                           |
|                   |                           | `nav_panel()`이 개별 페이지               |
+-------------------+---------------------------+-------------------------------------------+
| 다중 패널         | `navset_tab()`            | `page_fluid()` 혹은 `page_sidebar()` 내부 |
|                   |                           |                                           |
| (페이지 레벨)     | `navset_pill()`           | `nav_panel()`이 개별 패널                 |
|                   |                           |                                           |
|                   | `navset_underline()`      |                                           |
+-------------------+---------------------------+-------------------------------------------+
| 다중 패널         | `navset_card_tab()`       | `card()` 내부                             |
|                   |                           |                                           |
| (카드 레벨)       | `navset_card_pill()`      | `nav_panel()`이 개별 패널                 |
|                   |                           |                                           |
|                   | `navset_card_underline()` |                                           |
+-------------------+---------------------------+-------------------------------------------+

: bslib 레이아웃 함수 {#tbl-bslib1 tbl-colwidths="\[25, 35, 40\]"}
